...
String.prototype.replace_all = (function(){
  function replace_all(f, r){
    var str, i;
    str = this;
    i = 0;
    while (!(i = str.indexOf(f, i === -1))) {
      str = str.replace(f, r);
      i += r.length;
    }
    return String(str);
...
exports.tokenize = function(code, o){
  ...
  while (c = code.charAt(i)) {
    ...
    case '<':
      c2 = code.charAt(i + 1);
      i += ('a' <= c2 && c2 <= 'z') || ('A' <= c2 && c2 <= 'Z') ? this.doTags(code, i) : void 8;
      break;
    case '/':
...
exports.doTags = function(code, index, q){
  var depth, parse, attributes, parser, i, c, _this = this;
  depth = 0;
  parse = true;
  attributes = 0;
  ...
  parser.onopentag = function(node){
    ...
    if (depth++) {
      if (attributes === 1) {
        ...
        _this.token('STRNUM', 0);
      }
      _this.token(',', ',');
      if (attributes === 2) {
        _this.token('TERMINATOR', "\n");
        attributes = 0;
      } else {
        _this.token('INDENT', 2);
      }
    }
    _this.token('ID', 'cE');
    _this.token('CALL(', '(');
    _this.token('STRNUM', "'" + node.name + "'");
    _this.token(',', ',');
    for (k in _ref = node.attributes) {
      ...
      if (isNaN(v * 1)) {
        v = '"""' + v + '""""';
        _results.push(_this.doString(v, 0, '"'));
      } else {
        _results.push(_this.token('STRNUM', String(v)));
      }
    }
    return _results;
  };
  ...
  parser.ontext = function(text){
    if (text.trim().length) {
      ...
      _this.token(',', ',');
      return _this.token('STRNUM', "'" + text.replace_all('\'', '\\\'') + "'");
    } else if (text === "\n") {
      return _this.token('DEDENT', 2);
    }
  };
  parser.onprocessinginstruction = function(node){
    return console.log('TODO processing nodes', node);
  };
