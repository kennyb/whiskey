var sax, string, TABS, INDENTS, LINES, BACKSLASHES, reslash, character, KEYWORDS_SHARED, KEYWORDS_UNUSED, KEYWORDS, COCO_ALIASES, ID, SYMBOL, SPACE, MULTIDENT, SIMPLESTR, JSTOKEN, BSTOKEN, NUMBER, NUMBER_OMIT, REGEX, HEREGEX_OMIT, LASTDENT, LINE_CONTINUER, OPENERS, CLOSERS, INVERSES, CHAIN, ARG, __clone = function(it){
...
exports.doTags = function(code, index, q){
  ...
  parser.onclosetag = function(node){
    var prev, scope, code_start, code_end, code_scope, i, v, s, _ref, _len, _i;
    depth--;
  ...
  parser.ontext = function(text){
    text = String(text);
    if (isNaN(text * 1)) {
      return add_scope(['STRNUM', '"""' + text.trim() + '"""']);
    } else {
      return add_scope(['STRNUM', text]);
    }
  };
  parser.onprocessinginstruction = function(node){
    return console.log('code', node);
  };
  parser.onend = function(){
    var tokens, i, s, t, _i, _len, _results = [];
    parse = 0;
    tokens = scopes[0];
    i = 0;
    ...
    for (_i = 0, _len = tokens.length; _i < _len; ++_i) {
      t = tokens[_i];
      if (t[0] === 'STRNUM' && t[1].substr(0, 3) === '"""') {
        _results.push(_this.doHeredoc(t[1], 0, '"'));
      } else {
        _results.push(_this.token.apply(_this, t));
      }
    }
    return _results;
  };
