var sax, string, TABS, unlines, enlines, enslash, reslash, character, KEYWORDS_SHARED, KEYWORDS_UNUSED, KEYWORDS, ID, SYMBOL, SPACE, MULTIDENT, SIMPLESTR, JSTOKEN, BSTOKEN, NUMBER, NUMBER_OMIT, REGEX, HEREGEX_OMIT, LASTDENT, OPENERS, CLOSERS, INVERSES, CHAIN, ARG, __indexOf = [].indexOf || function(x){
...
exports.rewrite = function(it){
  ...
  expandLiterals(it);
  if (((_ref = it[0]) != null ? _ref[0] : void 8) === 'NEWLINE') {
    it.shift();
  }
  return it;
};
exports.tokenize = function(code, o){
  ...
  while (c = code.charAt(i)) {
    ...
    default:
      c2 = code.charAt(i + 1);
      if (c === '<' && (('a' <= c2 && c2 <= 'z') || ('A' <= c2 && c2 <= 'Z'))) {
        i += this.doTags(code, i);
      } else {
    ...
    case 'of':
      if (this.seenFor) {
        ...
        if (id === 'of') {
          id = '';
          this.wantBy = true;
          if (last[0] === 'ID' && (_ref = this.tokens)[_ref.length - 2][0] !== 'FOR') {
            id = this.tokens.pop()[1];
            if ((_ref = this.tokens)[_ref.length - 1][0] === ',') {
              this.tokens.pop();
            }
          }
        }
...
exports.doTags = function(code, index, q){
  var xml_func, parse, attributes, parser, depth, scopes, idx, add_scope, len, c, _this = this;
  xml_func = 'cE';
  parse = true;
  attributes = 0;
  ...
  add_scope = function(token, opts){
    ...
    if (opts && opts.code) {
      if (scopes[depth].code_start >= 0) {
        ...
        return scopes[depth].code_end = last;
      } else {
        scopes[depth].code_blocks = [last];
        return scopes[depth].code_start = last;
      }
    }
  };
  parser.onopentag = function(node){
    var attributes, k, v, tokens, a, _ref, _i, _len;
    attributes = [];
    for (k in _ref = node.attributes) {
      v = _ref[k];
      switch (k) {
      case 'class':
        k = 'c';
    ...
    if (typeof scopes[++depth] !== 'array') {
      return scopes[depth] = [];
    }
  };
  parser.onclosetag = function(node){
    var prev, scope, code_start, code_end, code_scope, i, v, s, tokens, t, _ref, _len, _i;
    depth--;
    prev = (_ref = scopes[depth])[_ref.length - 1];
    ...
    if (scope.length) {
      if (scope.code_start >= 0) {
        if (isNaN(scope.code_end)) {
          ...
          scope[code_start].unshift({
            tokens: [['UNARY', 'do'], ['PARAM', '('], ['ID', 'o'], [')PARAM', ') ->'], ['INDENT', 1], ['ID', 'o'], ['ASSIGN', '= []'], ['TERMINATOR', '\n']]
          });
          code_scope = scope.splice(code_start + 1, code_end - code_start);
          for (i = 0, _len = code_scope.length; i < _len; ++i) {
            v = code_scope[i];
            if (i === code_scope.length - 1) {
              scope[code_start].push({
                tokens: [v, ['HURL', 'return'], ['ID', 'o']]
              });
            } else if (scope.code_blocks.indexOf(i) !== -1) {
              scope[code_start].push({
                tokens: [v, ['ID', 'o'], ['DOT', '.'], ['ID', 'push'], ['CALL(', '('], v, [')CALL', ')'], ['TERMINATOR', '\n']]
              });
            } else {
    ...
    scopes[depth + 1] = [];
    tokens = scopes[depth].concat([[')CALL', ')CALL']]);
    i = 0;
    while (i < tokens.length) {
      s = tokens[i];
      if (s.tokens) {
        tokens.splice.apply(tokens, [i, 1].concat(s.tokens));
      } else {
        i++;
      }
    }
    for (_i = 0, _len = tokens.length; _i < _len; ++_i) {
      t = tokens[_i];
      if (t[0] === 'STRNUM' && typeof t[1] === 'string' && t[1].substr(0, 3) === '"""') {
        _this.doHeredoc(t[1], 0, '"');
      } else {
        _this.tokens.push(t);
      }
    }
  ...
  parser.ontext = function(text){
    ...
    tl = text.trimLeft();
    tr = text.trim();
    if (diff = text.length - tl.length) {
      this.doLine(text.substr(0, diff), 0);
    }
    console.log("diff_l", diff);
    if (tr.length) {
      if (isNaN(text * 1)) {
        add_scope(['STRNUM', '"""' + tr + '"""']);
      } else {
    ...
    if (diff = tl.length - tr.length) {
      this.doLine(tl.substr(-diff), 0);
    }
    return console.log("diff_r", diff);
  };
  parser.onprocessinginstruction = function(node){
    var body, tabs, nested, _ref;
    body = node.body.trim();
    switch (node.name) {
    case '=':
      return add_scope(['STRNUM', '"""#{' + body + '}"""']);
    default:
      tabs = heretabs(body.replace(LASTDENT, ''));
      console.log("tabs:", tabs);
      if (tabs) {
        console.log(body);
        body = String.repeat(body.charAt(body.indexOf("\n") + 1), tabs) + body;
      }
      nested = exports.tokenize(body, {
        line: _this.line,
        raw: true
      });
      console.log("finished", nested);
      while (((_ref = nested[0]) != null ? _ref[0] : void 8) === 'TERMINATOR') {
        nested.shift();
      }
      while (((_ref = nested[nested.length - 1]) != null ? _ref[0] : void 8) === 'TERMINATOR') {
        nested.pop();
      }
  ...
  console.log(this.tokens.slice(len));
  return idx - index;
};
...
exports.doLine = function(code, index){
  var input, tabs, length, last, delta, that, tag, _ref;
  ...
  if (0 > (delta = tabs.length - this.dent)) {
    ...
    if ((tag = last[0]) === 'ASSIGN' && ((_ref = '' + last[1]) !== '=' && _ref !== ':=' && _ref !== '+=') || (tag === '+-' || tag === '=>' || tag === 'DOT' || tag === 'LOGIC' || tag === 'MATH' || tag === 'COMPARE' || tag === 'RELATION' || tag === 'SHIFT' || tag === 'BITWISE' || tag === 'IN' || tag === 'OF' || tag === 'TO' || tag === 'BY' || tag === 'FROM' || tag === 'EXTENDS')) {
      return length;
    }
    if (delta) {
      this.indent(delta);
...
exports.doLiteral = function(code, index){
  ...
  case '**=':
    ...
    } else if ((val === '+=' || val === '-=' || val === '^=') && !able(this.tokens) && ((_ref = this.last[0]) !== '+-' && _ref !== '^' && _ref !== 'UNARY' && _ref !== 'LABEL')) {
    ...
    break;
  case '*':
    if (that = ((_ref = this.last[0]) === 'NEWLINE' || _ref === 'INDENT' || _ref === 'THEN') && /^.[^\n\S]*(?=\S)/.exec(code.slice(index))) {
...
exports.newline = function(){
  var _ref;
  return this.last[1] === '\n' || this.tokens.push(this.last = (_ref = ['NEWLINE', '\n', this.line], _ref.spaced = true, _ref));
};
exports.unline = function(){
  var _ref;
  switch (this.last[0]) {
  case 'INDENT':
    (_ref = this.dents)[_ref.length - 1] += '';
    // fallthrough
  case 'NEWLINE':
    this.tokens.length--;
  }
...
exports.interpolate = function(str, idx, end){
  ...
  while (ch = str.charAt(++i)) {
    ...
    if (id) {
      ...
      while (((_ref = nested[0]) != null ? _ref[0] : void 8) === 'NEWLINE') {
        nested.shift();
      }
...
exports.forange = function(){
  var _ref;
  return ((_ref = (_ref = this.tokens)[_ref.length - 2]) != null ? _ref[0] : void 8) === 'FOR' && (this.seenFor = false, this.seenFrom = true, this);
};
...
function addImplicitParentheses(tokens){
  ...
  while (token = tokens[++i]) {
    ...
    if (token[1] === 'do' && ((_ref = tokens[i + 1]) != null ? _ref[0] : void 8) === 'INDENT') {
      ...
      if (tokens[endi + 1][0] === 'NEWLINE' && ((_ref = tokens[endi + 2]) != null ? _ref[0] : void 8) === 'WHILE') {
        token[0] = 'DO';
  ...
  function ok(token, i){
    var pre, _ref;
    if (!skipBlock && token.alias && ((_ref = token[1]) === '&&' || _ref === '||') || token[0] === '=>') {
      return true;
    }
    pre = tokens[i - 1];
    switch (token[0]) {
...
function addImplicitBraces(tokens){
  ...
  while (token = tokens[++i]) {
    ...
    tokens.splice(idx, 0, ['{', '{', tokens[idx][2]]);
    detectEnd(tokens, ++i + 1, ok, go);
  }
  function ok(token, i){
    var tag, t1, _ref;
    ...
    return t1 !== (tag === ',' ? 'NEWLINE' : 'COMMENT') && ':' !== ((_ref = tokens[t1 === '('
